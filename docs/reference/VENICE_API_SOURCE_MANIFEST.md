...
| **Chat Completions (`/chat/completions`)** | `swagger.yaml` (`POST /chat/completions`), `guides/projects/rust-llm-gateway.mdx` | `venice-sdk/.../chat/ChatClient.kt`, `ChatRequest.kt`, `SseLineParser.kt` | `ChatClientTest.kt`, `SseLineParserTest.kt`, `ChatViewModelTest.kt` | **VERIFIED FOUNDATION**: cancellation-native async OkHttp SSE transport, multiline event parsing, multi-turn context, multiple tool-call deltas, typed HTTP failures, and abnormal-EOF rejection; non-streaming typed completion remains planned |
